...
public class PraxisFlowMain {
        ...
        while (isItRunning) {
            ...
            switch (choice) {
                ...
                case 3:
                    ...
                    break;
                default:
                    System.out.println("Invalid input please input a number between 1 - 3. \n");
            }
        }
        input.close();
    }
    ...
    private static void userRegisterMenu() {
        ...
        if (email.matches(studentRegex)) {
            ...
            if (password.equals(confirmPassword)) {
                ...
                stInterns.add(new StudentIntern(srCode, name, email, password, schoolName));
    ...
    private static void assignTaskToInternMenu() {
        ...
        if (stInternIndex >= 0 && stInternIndex < stInterns.size()) {
            stInterns.get(stInternIndex).receiveTask(tasks);
    ...
    private static void logHoursForStudentIntern(StudentIntern studentIntern) {
        ...
        if (date.matches(yearMothsDayRegex)) {
            studentIntern.logData(hours, date);
        } else {
    ...
    private static void approveInternshipMenu(AdminMain admin) {
        ...
        for (int i = 0; i < pendingInternships.size(); i++) {
            ...
            System.out.println((i + 1) + ". " + internship);

            // Show student info if assigned
            if (internship.getIntern() != null) {
                System.out.println("   Student: " + internship.getIntern().getName() +
                        " (Current Status: " + internship.getIntern().getStatus() + ")");
            } else {
                System.out.println("   Student: Not assigned yet");
            }
            System.out.println();
        }
        System.out.println("=============================================");

        int index = getInputInteger("Select internship to approve (0 to cancel): ") - 1;
        ...
        if (index >= 0 && index < pendingInternships.size()) {
            System.out.println();
            System.out.println("1. Approve Internship");
            System.out.println("2. Reject Internship");
            int action = getInputInteger("Enter your choice (1-2): ");

            switch(action) {
                case 1: admin.approveInternship(pendingInternships.get(index));
                break;
                case 2:
                    System.out.print("Enter reason for rejection: ");
                    String reason = input.nextLine();
                    admin.rejectInternship(pendingInternships.get(index), reason);
                break;
                default:
    ...
    private static void recommendStudentMenu(School school) {
        ...
        int internIndex = getInputInteger("Select Intern: ") - 1;
    ...
    private static void monitorProgressMenu(School school) {
        ...
        int index = getInputInteger("Select Student Intern: ") - 1;
